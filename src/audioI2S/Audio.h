...
inline static constexpr std::array<const char*, 13> eventStr = {"info",    "id3data",  "eof",      "station_name", "icy_description", "streamtitle", "bitrate",
...
class AudioBuffer {
  ...
  protected:
    ...
    uint8_t*        m_write_ptr = nullptr;
    uint8_t*        m_read_ptr = nullptr;
    uint8_t*        m_end_ptr = nullptr;
    uint8_t*        m_start_ptr = nullptr;
    bool            m_init = false;
    bool            m_isEmpty = true;
    bool            m_isFull = false;
...
// —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
size_t process_m3u8_ID3_header(uint8_t* packet); // Dodanie brakującej deklaracji funkcji
