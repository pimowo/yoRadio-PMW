...
void Player::loop()
{
  ...
  if (xQueueReceive(playerQueue, &requestP, isRunning() ? PL_QUEUE_TICKS : PL_QUEUE_TICKS_ST))
  {
    ...
    case PR_CHECKSD:
    {
      ...
      {
        // If SD not present, stop manager and show message. If SD manager
        // isn't ready, try to remount occasionally (debounce controlled
        // by sd_last_mount_attempt/SD_MOUNT_RETRY_MS).
        if (!sdman.cardPresent())
        {
          ...
          if ((now - sd_last_mount_attempt) >= SD_MOUNT_RETRY_MS) {
            Serial.println("PR_CHECKSD: attempting SD remount");
#if SRC_SD
            if (digitalRead(SD_DETECT_PIN) == HIGH) {
              Serial.println("PR_CHECKSD: detect pin says no SD card");
              sd_last_mount_attempt = now;
              sdman.stop();
            } else
#endif
            if (sdman.start()) {
              Serial.println("PR_CHECKSD: SD remount success");
              // Start indexing task if user is on SD screen
              if (display.mode() == SDCHANGE) {
                startSdIndexTask();
              }
            } else {
              sd_last_mount_attempt = now;
              Serial.println("PR_CHECKSD: SD remount failed");
              sdman.stop();
            }
          }
        }
...
void Player::_play(uint16_t stationId)
{
  ...
  if (isConnected)
  {
    ...
    // Ensure display shows updated station/metadata when playback starts
