...
void bluetooth_handle_line(const char *line)
{
    ...
    if (strcmp(cmd, "TITLE") == 0)
    {
        ...
        if (config.getMode() == PM_BLUETOOTH)
        {
            display.putRequest(NEWTITLE);
            char meta[256];
            snprintf(meta, sizeof(meta), "%.125s - %.125s", local.artist, local.title); // Truncate artist and title to fit buffer
    ...
    if (strcmp(cmd, "PLAYING") == 0)
    {
        ...
        if (config.getMode() == PM_BLUETOOTH)
        {
            ...
            display.putRequest(DBITRATE);
            display.putRequest(NEWTITLE);
            // ensure play/pause state is reflected immediately
    ...
    if (strcmp(cmd, "STOPPED") == 0)
    {
        ...
        if (config.getMode() == PM_BLUETOOTH)
        {
            ...
            telnet.printf("##CLI.META#: \r\n");
                    display.putRequest(DBITRATE);
                    display.putRequest(NEWTITLE);
                    // ensure UI shows stopped state immediately
