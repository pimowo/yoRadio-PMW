...
void handleFm(AsyncWebServerRequest *request);
void handleFmStations(AsyncWebServerRequest *request);
...
bool NetServer::begin(bool quiet)
{
  ...
  webserver.on("/fm", HTTP_GET, handleFm);
  webserver.on("/fm", HTTP_POST, handleFm);
  webserver.on("/fmstations", HTTP_GET, handleFmStations);
...
void NetServer::processQueue()
{
  ...
  if (xQueueReceive(nsQueue, &request, NS_QUEUE_TICKS))
  {
    ...
    case STATIONNAME:
    {
      ...
      {
        snprintf(wsBuf, sizeof(wsBuf), "{\"payload\":[{\"id\":\"nameset\", \"value\": \"%s\"}]}", SRC_AUX3_NAME);
      }
      else
      {
    ...
    case TITLE:
    {
      ...
      {
        snprintf(wsBuf, sizeof(wsBuf), "{\"payload\":[{\"id\":\"meta\", \"value\": \"%s\"}]}", SRC_AUX3_NAME2);
        telnet.printf("##CLI.META#: %s\r\n> ", SRC_AUX3_NAME2);
      }
      else
      {
...
void handleFm(AsyncWebServerRequest *request)
{
  Serial.printf("[FM] handleFm called, method: %s\n", request->method() == HTTP_GET ? "GET" : "POST");
  Serial.printf("[FM] network.status: %d\n", network.status);
  if (request->method() == HTTP_GET) {
    if (network.status == CONNECTED)
    {
      Serial.println("[FM] Sending FM page");
      AsyncWebServerResponse *response = request->beginResponse_P(200, "text/html", fm_html);
      response->addHeader("Cache-Control", "no-cache");
      request->send(response);
    }
    else
    {
      Serial.println("[FM] Not connected, redirecting to settings");
      request->redirect("/settings.html");
    }
  } else if (request->method() == HTTP_POST) {
    Serial.println("[FM] Handling POST");
    // Handle FM stations upload
    if (request->hasParam("fmstations", true)) {
      String fmData = request->getParam("fmstations", true)->value();
      Serial.printf("[FM] Received data: %s\n", fmData.c_str());
      if (SPIFFS.exists(FMSTATIONS_PATH)) {
        SPIFFS.remove(FMSTATIONS_PATH);
      }
      File file = SPIFFS.open(FMSTATIONS_PATH, "w");
      if (file) {
        file.print(fmData);
        file.close();
        config.loadFmStations(); // Reload FM stations
        Serial.println("[FM] FM stations saved and reloaded");
        request->send(200, "text/plain", "FM stations saved");
      } else {
        Serial.println("[FM] Failed to open file");
        request->send(500, "text/plain", "Failed to save FM stations");
      }
    } else {
      request->send(400, "text/plain", "No FM stations data");
    }
  }
}
...
void handleFmStations(AsyncWebServerRequest *request)
{
  Serial.println("[FM] handleFmStations called");
  if (request->method() == HTTP_GET) {
    if (SPIFFS.exists(FMSTATIONS_PATH)) {
      Serial.println("[FM] Sending FM stations file");
      request->send(SPIFFS, FMSTATIONS_PATH, "text/plain");
    } else {
      Serial.println("[FM] FM stations file not found, sending empty");
      request->send(200, "text/plain", "");
    }
  }
}
