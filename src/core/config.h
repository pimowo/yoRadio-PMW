...
#define FMSTATIONS_PATH "/data/data/fmstations.csv"
...
struct config_t {
  ...
  // TDA7719 persistent mapping (working fields mirror TDASettings)
  struct tda7719_cfg_t {
    int16_t masterVolume;
    bool mute;
    int8_t balance;
    int8_t fader;
    int8_t bassGain;
    uint8_t bassCenterIdx;
    uint8_t bassQIdx;
    bool bassDC;
    int8_t midGain;
    uint8_t midCenterIdx;
    uint8_t midQIdx;
    int8_t trebleGain;
    uint8_t trebleCenterIdx;
    int8_t loudnessLevel;
    uint8_t loudnessFreqIdx;
    bool loudnessHighBoost;
    int16_t outL;
    int16_t outR;
    int16_t outSub;
    uint8_t subFilterIdx;
    uint8_t softStepTimeIdx;
    bool softMute;
    InputCfg inputs[5];
    bool inputInvert[5];
    bool dcDetect;
...
public:
  ...
  void loadFmStations();
  uint16_t getFmStationCount();
  float getFmStationFreq(uint16_t index);
  uint16_t getCurrentFmStation();
  void setCurrentFmStation(uint16_t val);
  FS *SDPLFS() {
...
// Bluetooth metadata shared across modules
typedef struct {
  uint32_t lastSeen;
  bool connected;
  bool playing;
  bool probeSent;
  uint32_t probeDeadline;
  bool awaitingAck;
...
// SD mount retry helpers (declared in config.cpp)
extern uint32_t sd_last_mount_attempt;
extern const uint32_t SD_MOUNT_RETRY_MS;
// Wrapper to start SD index task (defined in config.cpp when USE_SD)
